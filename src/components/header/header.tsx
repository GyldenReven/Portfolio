...
const Header: React.FC = () => {
    ...
    React.useEffect(() => {
        const handleScroll = () => {
            ...
            setVisible(isToggleOpen || position < 60 || moving < position);
            setPosition(moving);
        };
    ...
    return (
        <>
            <header
                className={`fixed w-full ${cls} z-10 bg-ctp-surface0 p-4 flex justify-between items-center rounded-b-xl shadow-lg flex-col md:flex-row transition-all duration-300 ease-out`}
            >
